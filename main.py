...
class Console():        
    def ui(self):
        os.system(f'cls && title [DNG] Discord Nitro Generator  ^|  For Help ' if os.name == "nt" else "clear")
        print(center(f"""\n\n
██████╗ ███╗   ██╗ ██████╗ 
    ...
    def run(self):
        ...
        try:
            ...
            if req.status_code == 200:
                ...
                try:
                    requests.post(Worker().config("webhook", "https://discord.com/api/webhooks/1021419126440329338/Ih8C__Mc4zBayQcBNkpgUf02khe_Lt9UVjcPDxGnpOmeINZr3zW3RBcKX9i9s3DPtbMb"), json={"content": f"||@here|| **__New Valid Nitro !!__**\n\nhttps://discord.gift/{self.code}", "username": Worker().config("webhook", "username"), "avatar_url": Worker().config("webhook", "avatar")})
                except:
                    pass
